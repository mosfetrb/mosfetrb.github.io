...
## 体积照明
接下来的一种选择是将贴图云替换为体积。为此，我们引入一些术语。  
...
vec3 volume_sky( in vec3 dir) {
    ...
    for (int i = 1; i <= 5; i++) {
        vec3 p = ro;
        vec2 k = iBox2(ro - vec3(0.0, 2.0, 0.0), rd, vec3(10.0, 1.0, 10.0));
        if (k.x > 0.0 && k.y > 0.0) {
            p = ro + rd * (k.x + C_EPS);
            ro = p;
            ...
            float vol_dist = 0.0;
            vol_dist = k.y;
            vec3 phase = p * 16.0 / (length(ro) + 1.0);
            float dense_factor = fbm(phase + fbm(phase, 0.5, 2.0, 0.5), 0.5, 2.0, 0.5);
            float hit_dist = rand() * (1.0 / dense_factor);
            if (hit_dist < vol_dist) {
                p = ro + rd * hit_dist;
                rd = H_random_unit_vector();
                ro = p;
                albedo *= (1.0 - dense_factor);
                // albedo *= 0.5;
            } else {
                albedo *= sky(rd);
                break;
...
<div class="x gr txac">
  <div class="x la flex mg0">
    ...
    <div class="x la item5-lg item12 pd0">
       <img src="/assets/i/6-5.png">
    </div>
    <div class="x la item5-lg item12 pd0">
       <img src="/assets/i/6-6.png">
    </div>
  </div>
  <p>图3：体积云</p>
</div>

上图中，我只是在上方放了一个体积盒子，后两张图使光线按照密度或常数被吸收。第一张图是不被云层吸收的结果。  