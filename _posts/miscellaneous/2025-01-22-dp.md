...
<nav class="content-nav" style="position:fixed;left:0;bottom:0;">
<a href="#top">返回/</a>
<a href="#原则">模式的模式/</a>
...
这是一种理想意见，尽量分离可能在相当长时间不会产生变化的部分，从而避免链式修改。  

**②抽象(接口)优先编程，后置事物的具体实现。**  
捕获事物存在的共同动作抽象--提取抽象化接口，再根据规范接口实现内容，而不是直接完成两种包含重复内容(未提取抽象)的类(例如飞机和火车)。  
笔者注：该"接口"术语的含义与硬件一致。  

**③组合优先而不是继承。**  
继承伴随着一些令人不愉快的附加条件。继承不为所有超类创建实例，相反，它创建了通过公共实现核心绑定在一起的两个类的引用。本质而言，通过事先将子类与超类本身的方法集结合，(从而使创建的子类实例)改变超类的行为。子类对超类的内部结构有独特的看法。超类的任何未仔细隐藏的内部运作对于子类来说都是清晰可见的。  
...
```
我们还增加了对引擎封装，使其成为一堵接口墙幕，而不是将所有引擎的细节暴露给Vehicle(并暴露给子类)。现在Car必须通过新的接口进行工作。  
另外，由于引擎只是一个内部对象，现在可以自由在生命周期内更换引擎，这在原始定义几乎不可能做到。  

**④委托。**  
当接口调用被实现为内部对象后，可以使用委托来调用原功能。  
```ruby
def start_engine
  @engine.start
end
...
超类可以定义任何要求内容，如果要子类选择的是具有算法流程之类的东西，则可以在超类中将每个阶段显示标记出来。  
```ruby
# 多格式报告输出
...
命令模式也出现在ActiveRecord Migration中。就像我们知道的，模式可以向上和向下取消。  
